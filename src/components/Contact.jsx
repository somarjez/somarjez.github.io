    ...
    <Section id="contact" title="Let's Connect" subtitle="Open to opportunities and collaborations">
      <div className="grid gap-10 md:grid-cols-2">
        <div className="space-y-4">
          ...
          {CONTACTS.map((c) => {
            const inner = (
              <div className="surface flex items-center gap-4 rounded-xl p-4 transition-colors hover:border-primary/40">
                <span className="grid h-11 w-11 place-items-center rounded-lg bg-primary/10 text-primary">
                  <i className={`${c.brand ? 'fab' : 'fas'} ${c.icon}`} />
                </span>
                <div>
                  <div className="font-mono text-sm font-semibold text-foreground">{c.label}</div>
        ...
        <form onSubmit={onSubmit} className="surface space-y-4 rounded-2xl p-6">
          <input name="name" required placeholder="Full Name" aria-label="Full Name"
            className="w-full rounded-lg border border-line bg-panel px-4 py-2.5 text-sm text-foreground outline-none placeholder:text-subtle focus:border-primary/60 focus:ring-1 focus:ring-primary/30" />
          ...
          <p role="status" aria-live="polite" className="text-center text-sm min-h-[1.25rem]">
            {status === 'sent' && <span className="text-green">Thanks — I'll get back to you soon!</span>}
